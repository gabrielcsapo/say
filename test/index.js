var test = require('tape').test;
var Say = require('../saywhat.js');

test('saywhat', function(t) {
    t.plan(9);

    t.test('should not output `hello`', function(t) {
        t.notEqual(Say.encode('hello'), 'hello');
        t.end();
    });

    t.test('should output `hello`', function(t) {
        var text = Say.encode('hello');
        var decoded = Say.decode(text);
        t.equal(decoded, 'hello');
        t.end();
    });

    t.test('should nout output `1234567890`', function(t) {
        t.notEqual(Say.encode('1234567890'), '1234567890');
        t.end();
    });

    t.test('should output `1234567890`', function(t) {
        var text = Say.encode('1234567890');
        var decoded = Say.decode(text);
        t.equal(decoded, '1234567890');
        t.end();
    });

    t.test('should nout output `hello1234567890`', function(t) {
        t.notEqual(Say.encode('hello1234567890'), 'hello1234567890');
        t.end();
    });

    t.test('should output `hello1234567890`', function(t) {
        var text = Say.encode('hello1234567890');
        var decoded = Say.decode(text);
        t.equal(decoded, 'hello1234567890');
        t.end();
    });

    t.test('should output large text', function(t) {
        var string = '';
        var text = Say.encode(string);
        var decoded = Say.decode(text);
        t.equal(decoded, string);
        t.end();
    });

    t.test('should work with \'', function(t) {
        var string = "when words aren't enough";
        var text = Say.encode(string);
        var decoded = Say.decode(text);
        t.equal(decoded, string);
        t.end();
    });

    t.test('should work with "', function(t) {
        var string = 'when words aren"t enough';
        var text = Say.encode(string);
        var decoded = Say.decode(text);
        t.equal(decoded, string);
        t.end();
    });

    t.end();
});
